...
public:
  ...
  HEPTopTagger(double mass_drop_threshold=0.8, double max_subjet_mass=30.,
               bool use_subjet_mass_cuts=false, double ptMin=20., double centralEtaCut=2.5)
    : _mass_drop_threshold(mass_drop_threshold),
      _max_subjet_mass(max_subjet_mass),
      _use_subjet_mass_cuts(use_subjet_mass_cuts),
		_ptMin(ptMin),
		_centralEtaCut(centralEtaCut)
  {}
  
  void run( const std::vector<fastjet::PseudoJet> & fjInputs, std::vector<CompoundPseudoJet> & fjJets, boost::shared_ptr<fastjet::ClusterSequence> & fjClusterSeq);
  ...
  virtual CompoundPseudoJet result(const PseudoJet & jet, boost::shared_ptr<fastjet::ClusterSequence> & fjClusterSeq) const;
